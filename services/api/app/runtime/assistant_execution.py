...
def _request_from_run(run: RuntimeRun) -> AssistantRequest:
    ...
    policy = (
        run.policy_snapshot_json if isinstance(run.policy_snapshot_json, dict) else {}
    )
    ...
    return AssistantRequest(
        ...
        if isinstance(source.get("client_request_id"), str)
        else None,
        project_id=run.project_id,
        conversation_id=run.conversation_id,
        provider_config_id=run.provider_config_id,
        reasoning_effort=run.reasoning_effort
        if run.reasoning_effort in {"low", "medium", "high", "extra", "max"}
        else None,
...
def _load_memory_context_in_session(
    user: CurrentUser,
    *,
    project_id: str | None,
    query: str,
):
    """Read optional memory with a thread-owned DB session."""

    with SessionLocal() as context_db:
        return load_authorized_memory(
            context_db,
            user,
...
async def execute_queued_assistant_run(
    ...
    *,
    ...
    if user_row is None:
        fail_runtime_run(
            db, run.id, "助手所属用户不存在。", error_code="assistant_user_missing"
        )
        return "failed"
    if not run.conversation_id:
        fail_runtime_run(
            ...
            run.id,
            "助手运行缺少会话上下文。",
            error_code="assistant_conversation_missing",
        )
        return "failed"
    ...
    source = run.input_json if isinstance(run.input_json, dict) else {}
    user_message_id = (
        source.get("user_message_id")
        if isinstance(source.get("user_message_id"), str)
        else None
    )
    ...
    if not payload.message.strip():
        fail_runtime_run(
            db, run.id, "助手运行缺少用户消息。", error_code="assistant_message_missing"
        )
        return "failed"
    ...
    memory_context, profile_context = await asyncio.gather(
        asyncio.to_thread(
            _load_memory_context_in_session,
            user,
            project_id=run.project_id,
    ...
    async for _event in stream_pi_assistant_response(
        ...
        memory_context_version=memory_context.memory_version
