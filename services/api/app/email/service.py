...
class SmtpEmailBackend:
    ...
    def send(self, message: EmailMessage) -> None:
        ...
        with smtp_cls(SMTP_HOST, SMTP_PORT) as server:
            ...
            server.sendmail(SMTP_FROM, [message.to], msg.as_string())
...
def send_password_reset_email(email: str, token: str) -> None:
    ...
    send_email(EmailMessage(
        ...
        body_text=(
            f"You requested a password reset for your DocPilot account.\n\n"
        ...
        body_html=(
            ...
            f"<p>You requested a password reset for your DocPilot account.</p>"
...
def send_email_verification_email(email: str, token: str) -> None:
    ...
    send_email(EmailMessage(
        ...
        body_text=(
            f"Welcome to DocPilot! Please verify your email address by clicking the link below.\n\n"
            f"{verify_url}\n\n"
            f"This link expires in 24 hours."
        ),
        body_html=(
            f"<h2>Verify Your Email</h2>"
            f"<p>Welcome to DocPilot! Please verify your email address.</p>"
...
def send_account_deletion_confirmation_email(email: str) -> None:
    ...
    send_email(EmailMessage(
        to=email,
        subject="DocPilot — Account Deleted",
        body_text=(
            f"Your DocPilot account and all associated data have been deleted.\n\n"
            f"If you did not request this, please contact support immediately."
        ),
        body_html=(
            f"<h2>Account Deleted</h2>"
            f"<p>Your DocPilot account and all associated data have been deleted.</p>"
...
def send_invitation_email(email: str, token: str, org_slug: str) -> None:
    ...
    send_email(EmailMessage(
        ...
        body_text=(
            f"You've been invited to join the '{org_slug}' organization on DocPilot.\n\n"
            f"To accept, register at: {link}\n\n"
            f"This invitation expires in 7 days."
        ),
        body_html=(
            f"<h2>You've been invited!</h2>"
            f"<p>You've been invited to join the <strong>{org_slug}</strong> organization on DocPilot.</p>"
