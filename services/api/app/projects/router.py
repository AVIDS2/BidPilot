...
from .repository import delete_project
from .schemas import ProjectCreate, ProjectRead, ProjectStatusUpdate
from .service import create_project_command, get_project_query, list_projects_query, update_project_status_command, delete_project_command
...
def get_project(project_id: str, db: Session = Depends(get_db)) -> ProjectRead:
    result = get_project_query(db, project_id)
    if result is None:
...
def update_project(project_id: str, payload: ProjectStatusUpdate, db: Session = Depends(get_db)) -> ProjectRead:
    result = update_project_status_command(db, project_id, payload.status)
    if result is None:
        raise HTTPException(status_code=404, detail="Project not found")
    return result
...
def delete_project_endpoint(project_id: str, db: Session = Depends(get_db)) -> None:
    if not delete_project_command(db, project_id):
        raise HTTPException(status_code=404, detail="Project not found")
