...
- Public-release secret review found no current-environment MiMo, OpenRouter or
  Supabase credential match in the working tree or in the history selected for
  publication, and no live key in current tracked source. It did find one old
  provider-key-shaped value in early development documentation; the release
  process removes it from every public ref and rescans after the rewrite. The
  value does not match the current environment. Provider-console
  revocation/rotation is outside repository permissions and remains the
  credential owner's responsibility; README and `SECURITY.md` state this
  without claiming provider-side revocation.
