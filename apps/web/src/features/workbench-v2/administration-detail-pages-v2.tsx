...
import { displayWorkbenchValue } from "./workbench-v2-labels";
...
export function UserManagementPageV2() {
    ...
    <section className="wb-admin-detail-page" aria-labelledby="users-page-title">
            ...
            {users.map((user) => {
              ...
              return (
                <div className="wb-operations-table-row" key={user.id}>
                  ...
                  <select
                    aria-label={`${user.display_name || user.email} 的角色`}
                    className="wb-inline-select"
                    disabled={isCurrentUser || roleMutation.isPending}
                    id={`role-${user.id}`}
                    onChange={(event) => roleMutation.mutate({ userId: user.id, role: event.target.value })}
                    value={user.role}
                  >
                    <option value="member">{displayWorkbenchValue("member")}</option>
                    <option value="admin">{displayWorkbenchValue("admin")}</option>
                  </select>
                  ...
                  <span className="wb-admin-row-actions">
                    {!isCurrentUser ? (
                      <button
                        className={cn("wb-row-text-action", user.disabled && "is-positive")}
      ...
      {totalPages > 1 ? (
        <footer className="wb-pagination" aria-label="用户分页">
          <button disabled={page <= 1} onClick={() => setPage((current) => current - 1)} type="button"><ChevronLeftIcon aria-hidden="true" /> 上一页</button>
          <span>{page} / {totalPages}</span>
          <button disabled={page >= totalPages} onClick={() => setPage((current) => current + 1)} type="button">下一页 <ChevronRightIcon aria-hidden="true" /></button>
        </footer>
      ) : null}
    </section>
  );
...
export function TeamManagementPageV2() {
  const { user: currentUser } = useAuth();
  const queryClient = useQueryClient();
    ...
    <ResizableSplitPage className="wb-admin-team-page" labelledBy="teams-page-title" storageKey="bidpilot.teams.split-width">
      <aside className="wb-split-list wb-admin-team-list">
        <header className="wb-admin-team-list-head">
          ...
          <button aria-label="创建团队" className="wb-flat-icon-button" disabled={!canManage} onClick={() => setShowCreate((value) => !value)} type="button"><UserPlusIcon aria-hidden="true" /></button>
        </header>
        {showCreate ? (
          <form className="wb-admin-create-form" onSubmit={submitCreate}>
            <label>团队名称<input autoFocus onChange={(event) => setName(event.target.value)} value={name} /></label>
            <label>标识<input onChange={(event) => setSlug(event.target.value)} placeholder="proposal-team" value={slug} /></label>
            <span><button className="wb-primary-button" disabled={createMutation.isPending} type="submit">创建</button><button className="wb-row-text-action" onClick={() => setShowCreate(false)} type="button">取消</button></span>
          </form>
        ) : null}
        ...
        <div className="wb-admin-team-list-rows">
          {teams.map((team) => (
            <button className={cn("wb-side-list-row", selectedTeam?.id === team.id && "is-selected")} key={team.id} onClick={() => setSelectedTeamId(team.id)} type="button">
          ...
          <>
            <header className="wb-admin-team-detail-head">
              {editingName ? (
                <form className="wb-admin-rename-form" onSubmit={submitRename}>
                  ...
                  <button className="wb-primary-button" disabled={renameMutation.isPending} type="submit"><CheckIcon aria-hidden="true" /> 保存</button>
                  <button className="wb-row-text-action" onClick={() => setEditingName(false)} type="button">取消</button>
                </form>
              ) : (
                <div><h2>{selectedTeam.name}</h2><p>{selectedTeam.slug}</p></div>
              )}
              <span className="wb-admin-row-actions">
                {!editingName ? <button className="wb-row-text-action" disabled={!canManage} onClick={() => { setNextName(selectedTeam.name); setEditingName(true); }} type="button">重命名</button> : null}
                <button className="wb-row-text-action is-danger" disabled={!canManage || deleteMutation.isPending} onClick={() => {
                  if (window.confirm(`删除团队“${selectedTeam.name}”？`)) deleteMutation.mutate(selectedTeam.id);
                }} type="button"><Trash2Icon aria-hidden="true" /> 删除</button>
              </span>
            </header>
                ...
                <div className="wb-operations-table wb-team-members-table">
                  ...
                  {selectedTeam.members.map((member) => (
                    <div className="wb-operations-table-row" key={member.id}>
                      ...
                      <span className="wb-admin-row-actions"><button className="wb-row-text-action is-danger" disabled={!canManage || removeMemberMutation.isPending} onClick={() => removeMemberMutation.mutate({ teamId: selectedTeam.id, userId: member.user_id })} type="button"><UserMinusIcon aria-hidden="true" /> 移出</button></span>
                    </div>
                  ))}
                </div>
            ...
            {canManage ? (
              <form className="wb-admin-add-member-form" onSubmit={(event) => {
                ...
                <select aria-label="选择要加入的组织成员" onChange={(event) => setNextMemberId(event.target.value)} value={nextMemberId}>
                  <option value="">选择组织成员</option>
                  {availableMembers.map((member) => <option key={member.id} value={member.id}>{member.display_name} · {member.email}</option>)}
                </select>
                <button className="wb-quiet-button" disabled={!nextMemberId || addMemberMutation.isPending} type="submit">加入团队</button>
              </form>
            ) : null}
          </>
        )}
...
export function InvitationManagementPageV2() {
  const { user: currentUser } = useAuth();
  const queryClient = useQueryClient();
  const membersQuery = useQuery({ queryKey: ["organization-members"], queryFn: listOrganizationMembers, staleTime: 20_000 });
  const canManage = canManageOrganization(membersQuery.data, currentUser?.id, currentUser?.role);
    ...
    <section className="wb-admin-detail-page" aria-labelledby="invitations-page-title">
      <header className="wb-page-header">
        ...
        <form className="wb-invite-form" onSubmit={submitInvite}>
          ...
          <button className="wb-primary-button" disabled={inviteMutation.isPending} type="submit"><MailPlusIcon aria-hidden="true" /> 邀请成员</button>
        </form>
      </header>
            ...
            {members.map((member) => {
              ...
              return (
                <div className="wb-operations-table-row" key={member.id}>
                  ...
                  <select aria-label={`${member.display_name} 的工作区角色`} className="wb-inline-select" disabled={isCurrentUser || roleMutation.isPending} onChange={(event) => roleMutation.mutate({ userId: member.id, role: event.target.value as OrganizationMemberRead["role"] })} value={member.role}>
                    <option value="member">{displayWorkbenchValue("member")}</option><option value="admin">{displayWorkbenchValue("admin")}</option><option value="owner">{displayWorkbenchValue("owner")}</option>
                  </select>
                  <span>{member.is_billing_owner ? <span className="wb-owner-label"><CrownIcon aria-hidden="true" /> 账单负责人</span> : "-"}</span>
                  <span className="wb-admin-row-actions">
                    {!member.is_billing_owner && !isCurrentUser ? <button className="wb-row-text-action" disabled={billingOwnerMutation.isPending} onClick={() => billingOwnerMutation.mutate(member.id)} type="button">设为账单 Owner</button> : null}
                    {!member.is_billing_owner && !isCurrentUser ? <button className="wb-row-text-action is-danger" disabled={removeMemberMutation.isPending} onClick={() => {
                      if (window.confirm(`将 ${member.display_name} 移出当前工作区？`)) removeMemberMutation.mutate(member.id);
                    }} type="button">移除</button> : null}
                  </span>
                </div>
              );
            })}
          ...
          <div className="wb-admin-table-scroll"><div className="wb-operations-table wb-invitation-admin-table">
            ...
            {invitations.map((invitation) => (
              <div className="wb-operations-table-row" key={invitation.id}>
                ...
                <span className="wb-admin-row-actions"><button className="wb-row-text-action is-danger" disabled={revokeMutation.isPending} onClick={() => revokeMutation.mutate(invitation.id)} type="button"><CircleOffIcon aria-hidden="true" /> 撤销</button></span>
              </div>
            ))}
          </div></div>
        ) : null}
