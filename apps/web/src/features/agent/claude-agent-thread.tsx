...
/* oxlint-disable nextjs/no-img-element -- previews use blob/data URLs that Next Image cannot optimize. */
import {
  CheckIcon,
  ChevronDownIcon,
  CopyIcon,
...
function ClaudeReasoning({
  ...
  return (
    ...
    >
      <CollapsibleTrigger type='button' className='cr-reasoning-heading'>
        <span className='cr-reasoning-status' aria-hidden='true'>
          <i />
        </span>
    ...
    <section className='cr-approval' aria-label='Approval required'>
      ...
      <div className='cr-approval-actions'>
        <Button
          type='button'
          disabled={!canConfirm}
          onClick={() => onConfirm(typedConfirmation)}
          size='sm'
          variant='default'
        >
          <CheckIcon size={14} />
...
function ClaudeAssistantMessage({
  ...
  const copyResponse = () => {
    const value =
      ...
      parts
        .filter(
          (part): part is Extract<typeof part, { kind: 'narrative' }> => part.kind === 'narrative'
        )
        .map((part) => part.text)
        .join('');
    ...
    >
      {hasTranscriptParts ? (
        <>
            ...
            if (part.kind === 'reasoning') {
              return isTimelineTitle(part) ? null : (
                <ClaudeReasoning
                  ...
                  part={part}
                  forceCompleted={
                    hasActiveExecution || !isStreaming || !isThinking || Boolean(sessionError)
                  }
                />
              );
            }
          ...
          {message.content &&
            !hasNarrativePart &&
            renderNarrative(message.content, `${message.id}-durable`)}
      ...
      {!isStreaming &&
        (message.content || parts.some((part) => part.kind === 'narrative' && part.text)) && (
...
export function ClaudeAgentThread({
  ...
  return (
    <div
      ref={threadRef}
      className='cr-thread'
      data-testid='claude-agent-thread'
