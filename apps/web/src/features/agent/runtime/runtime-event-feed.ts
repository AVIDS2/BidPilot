...
export function recoverRuntimeMessageFromEvents(
  ...
  const deltas = events
    ...
    .map((event) =>
      typeof event.payload?.delta === 'string' ? event.payload.delta : event.public_summary
    )
    .join('')
    .trim();
  ...
  switch (event.type) {
    ...
    case 'plan.updated': {
      ...
      if (stage === 'task_started') {
        const taskTitle =
          asString(payload.title) ?? asString(payload.task_title) ?? event.public_summary;
        return [
          {
    ...
    case 'capability.succeeded': {
      const result = Object.fromEntries(
        Object.entries(payload).filter(
          ([key]) => !['capability', 'action_id', 'turn_id', 'tool_call_id', 'title'].includes(key)
        )
      );
    ...
    case 'capability.progressed': {
      if (capability === 'deep_research') {
        const progress = Object.fromEntries(
          Object.entries(payload).filter(
            ([key]) =>
              !['capability', 'action_id', 'turn_id', 'tool_call_id', 'title'].includes(key)
          )
        );
        return [
          {
