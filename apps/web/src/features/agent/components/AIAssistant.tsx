...
export function AIAssistant() {
  ...
  const handleSend = useCallback(
    async (text?: string) => {
      ...
      try {
        const response = await fetch('/api/bidpilot/chat/stream', {
          method: 'POST',
          credentials: 'include',
          headers: {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === aiMessage.id ? { ...msg, content: '抱歉，发生了错误，请稍后重试。' } : msg
          )
        );
      } finally {
        setIsLoading(false);
              ...
              messages.map((message) => (
                ...
                >
                  <div
                    ...
                    style={{
                      background: message.role === 'user' ? 'var(--primary)' : 'var(--muted)',
                      color: message.role === 'user' ? 'white' : 'var(--foreground)'
                    }}
                  >
                    {message.content ||
                      (isLoading && message.id.startsWith('ai-') ? (
