...
const TOOL_FALLBACK_LABELS: Record<string, string> = {
  ...
  propose_memory: '记住这条信息',
  forget_memory: '忘记这条信息',
...
const TOOL_ICONS: Record<string, LucideIcon> = {
  ...
  discover_remote_documents: FileSearchIcon,
  ...
  start_deep_research: FileSearchIcon,
  read_skill_resource: BookOpenCheckIcon,
...
export function getAssistantToolLabel(toolName: string | undefined, t: Translate) {
  ...
  return t(`activity.tool.${toolName}`, {
    defaultValue:
      TOOL_FALLBACK_LABELS[toolName] ?? t('activity.tool.default', { defaultValue: '平台操作' })
  });
}
